...
#X1 = database[database["ENGINESIZE"] != np.nan]["ENGINESIZE"]
#y1 = database[database["CYLINDERS"] != np.nan]["CYLINDERS"]
#X2 = database[database["FUELCONSUMPTION_COMB"] != np.nan]["FUELCONSUMPTION_COMB"]
#y2 = database[database["CO2EMISSIONS"] != np.nan]["CO2EMISSIONS"]
...
plt.title("Datos de entrenamiento (train)")
plt.grid(True)
...
logging.info("El modelo linear fue entrenado con X_train.")
...
plt.xlabel("Fuel Consumption (ombined)")
